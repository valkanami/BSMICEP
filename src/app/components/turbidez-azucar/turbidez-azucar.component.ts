...
@Component({
  selector: 'app-turbidez-azucar',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './turbidez-azucar.component.html',
  styleUrls: ['./turbidez-azucar.component.css']
})
export class TurbidezAzucarComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public categories: string[] = ['1ER_TURNO', '2DO_TURNO'];
  ...
  checkApiConnection(): void {
    this.http.get('http://localhost:3000/api/turbidezazucar').subscribe({
      next: (response) => {
  ...
  private initChart(): void {
    ...
    try {
      ...
      this.chart = new Chart(ctx, {
        type: 'bar',
        data: {
          labels: ['Primer Turno', 'Segundo Turno'],
          datasets: [
            {
              label: 'Turbidez de Azúcar',
              data: values,
              backgroundColor: [
                'rgba(255, 99, 132, 0.7)',
                'rgba(54, 162, 235, 0.7)'

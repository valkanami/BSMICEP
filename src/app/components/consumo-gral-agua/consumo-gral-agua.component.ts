...
@Component({
  selector: 'app-consumo-gral-agua',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './consumo-gral-agua.component.html',
  styleUrls: ['./consumo-gral-agua.component.css']
})
export class ConsumoGralAguaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Consumo general agua lts')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Consumo general agua lts') {

...
@Component({
  selector: 'app-rendimiento-cristales-semana',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './rendimiento-cristales-semana.component.html',
  styleUrls: ['./rendimiento-cristales-semana.component.css']
})
export class RendimientoCristalesSemanaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Rendimiento Cristales')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Rendimiento Cristales') {

...
@Component({
  selector: 'app-descarga-gral-agua',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './descarga-gral-agua.component.html',
  styleUrls: ['./descarga-gral-agua.component.css']
})
export class DescargaGralAguaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Descarga general agua lts')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Descarga general agua lts') {

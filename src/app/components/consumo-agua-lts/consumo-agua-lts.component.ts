...
@Component({
  selector: 'app-consumo-agua-lts',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './consumo-agua-lts.component.html',
  styleUrls: ['./consumo-agua-lts.component.css']
})
export class ConsumoAguaLtsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Consumo de agua lts') {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Consumo de agua lts')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {

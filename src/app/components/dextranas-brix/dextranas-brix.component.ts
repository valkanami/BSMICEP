...
@Component({
  selector: 'app-dextranas-brix',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './dextranas-brix.component.html',
  styleUrls: ['./dextranas-brix.component.css']
})
export class DextranasBrixComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public limits: Limit[] = [
    { id: 43, name: '', value: null, color: 'rgba(255, 99, 132, 1)', axis: 'y', unit: '' },
  ];
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Dextranas brix')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Dextranas brix') {

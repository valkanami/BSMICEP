...
export class ColorComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public limitValue: number = 100; // Valor por defecto
  ...
  private loadLimitValue(): void {
    this.http.get('http://localhost:3000/api/limites/19').subscribe({
      next: (response: any) => {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData.map(item => {
      ...
      if (item.FECHA) {
        const date = new Date(item.FECHA);
        if (!isNaN(date.getTime())) {
          horaOriginal = this.formatTimeToHHMM(date);
        }
      }
      ...
      return {
        ...
        COLOR_AZUCAR_A: item.COLOR_AZUCAR_A || null,
  ...
  private initChart(): void {
    ...
    try {
      ...
      const colorAzucarData = this.mapDataToFixedHours('COLOR_AZUCAR_A');
  ...
  public updateChartData(): void {
    ...
    this.chart.data.datasets[0].data = this.mapDataToFixedHours('COLOR_AZUCAR_A');

...
@Component({
  selector: 'app-frescura-rsd',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './frescura-rsd.component.html',
  styleUrls: ['./frescura-rsd.component.css']
})
export class FrescuraRsdComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Frescura caña vs RSD') {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Frescura caña vs RSD')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {

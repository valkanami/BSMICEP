...
@Component({
  selector: 'app-ph-descarga-fabrica',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './ph-descarga-fabrica.component.html',
  styleUrls: ['./ph-descarga-fabrica.component.css']
})
export class PhDescargaFabricaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Ph descarga fabrica') {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Ph descarga fabrica')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {

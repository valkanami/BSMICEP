...
@Component({
  selector: 'app-pol-descarga-fabrica',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './pol-descarga-fabrica.component.html',
  styleUrls: ['./pol-descarga-fabrica.component.css']
})
export class PolDescargaFabricaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public limits: Limit[] = [
    { id: 51, name: '', value: null, color: 'rgba(255, 99, 132, 1)', axis: 'y', unit: '' },
  ];
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Pol descarga fabrica') {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Pol descarga fabrica')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {

...
@Component({
  ...
  templateUrl: './consumo-bagazo-astilla.component.html',
  styleUrls: ['./consumo-bagazo-astilla.component.css']
})
export class ConsumoBagazoAstillaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Consumo-bagazo-astilla')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Consumo-bagazo-astilla') {
  ...
  private prepareTableData(): void {
    const concepts = [...new Set(this.filteredData.map(item => item.dato))];

    this.productionData = concepts.map(concept => {
      const valores = this.daysOfWeek.map((_, dayIndex) => {
        const dayData = this.filteredData.filter(
          item => item.dayOfWeek === dayIndex && item.dato === concept
        );
        
        if (dayData.length > 0) {
          const sum = dayData.reduce((total, item) => total + (item.valor || 0), 0);
          const average = sum / dayData.length;
          return average % 1 === 0 ? average.toString() : average.toFixed(2);
        }
        return '0';
      });

...
@Component({
  selector: 'app-pol-cana',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './pol-cana.component.html',
  styleUrls: ['./pol-cana.component.css']
})
export class PolCanaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public limits: Limit[] = [
    { id: 44, name: '', value: null, color: 'rgba(255, 99, 132, 1)', axis: 'y', unit: '' },
  ];
  ...
  public readonly APARTADO_FILTRADO = 'Extracción pol % pol en caña';
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Ext. pol % pol en caña')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Ext. pol % pol en caña') {

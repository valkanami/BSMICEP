...
@Component({
  selector: 'app-comparativo-abl',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './comparativo-abl.component.html',
  styleUrls: ['./comparativo-abl.component.css']
})
export class ComparativoAblComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Comparativo Astilla Bagazo Leña') {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Comparativo Astilla Bagazo Leña')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {

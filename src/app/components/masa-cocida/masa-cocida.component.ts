...
@Component({
  selector: 'app-masa-cocida',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './masa-cocida.component.html',
  styleUrls: ['./masa-cocida.component.css']
})
export class MasaCocidaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Masa Cocida')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Masa Cocida') {

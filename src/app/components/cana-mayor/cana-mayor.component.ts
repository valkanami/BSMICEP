...
@Component({
  selector: 'app-cana-mayor',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './cana-mayor.component.html',
  styleUrls: ['./cana-mayor.component.css']
})
export class CanaMayorComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public limits: Limit[] = [
        { id: 56, name: '', value: null, color: 'rgba(255, 99, 132, 1)', axis: 'y', unit: '' },
  ];
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Caña 48 hrs')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Caña 48 hrs') {

...
@Component({
  ...
  templateUrl: './cachaza-cana.component.html',
  styleUrls: ['./cachaza-cana.component.css']
})
export class CachazaCanaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public limits: Limit[] = [
    { id: 45, name: '', value: null, color: 'rgba(255, 99, 132, 1)', axis: 'y', unit: '' },
    { id: 46, name: '', value: null, color: 'rgba(54, 162, 235, 1)', axis: 'y', unit: '' },
  ];
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Cachaza % caña')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Cachaza % caña') {

...
@Component({
  selector: 'app-ton-pol-cachaza',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './ton-pol-cachaza.component.html',
  styleUrls: ['./ton-pol-cachaza.component.css']
})
export class TonPolCachazaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Ton pol de cachaza')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Ton pol de cachaza') {

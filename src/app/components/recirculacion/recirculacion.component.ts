...
@Component({
  selector: 'app-recirculacion',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './recirculacion.component.html',
  styleUrls: ['./recirculacion.component.css']
})
export class RecirculacionComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Recirculacion')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Recirculacion') {

...
@Component({
  selector: 'app-color-fcr',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './color-fcr.component.html',
  styleUrls: ['./color-fcr.component.css']
})
export class ColorFcrComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  checkApiConnection(): void {
    this.http.get('http://localhost:3000/api/color').subscribe({
      next: (response) => {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData.map(item => {
      return {
        ...
        COLOR_FUNDIDO: item.COLOR_FUNDIDO || null,
        COLOR_CLARIF: item.COLOR_CLARIF || null,
        COLOR_REFINADO: item.COLOR_REFINADO || null,
  ...
  private initChart(): void {
    ...
    try {
      ...
      const fundidoData = this.mapDataToTurnos('COLOR_FUNDIDO');
      const clarifData = this.mapDataToTurnos('COLOR_CLARIF');
      const refinadoData = this.mapDataToTurnos('COLOR_REFINADO');
  ...
  public updateChartData(): void {
    ...
    this.chart.data.datasets[0].data = this.mapDataToTurnos('COLOR_FUNDIDO');
    this.chart.data.datasets[1].data = this.mapDataToTurnos('COLOR_CLARIF');
    this.chart.data.datasets[2].data = this.mapDataToTurnos('COLOR_REFINADO');

...
@Component({
  ...
  templateUrl: './cana-molida.component.html',
  styleUrls: ['./cana-molida.component.css']
})
export class CanaMolidaComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Caña Molida')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Caña Molida') {

...
@Component({
  selector: 'app-consumo-agua-m3',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './consumo-agua-m3.component.html',
  styleUrls: ['./consumo-agua-m3.component.css']
})
export class ConsumoAguaM3Component implements OnInit, AfterViewInit, OnDestroy {
  ...
  private extractDataTypes(): void {
    ...
    this.originalData.forEach(item => {
      if (item.dato && item.apartado === 'Consumo de agua m3') {
  ...
  private preserveOriginalTimes(rawData: any[]): any[] {
    return rawData
      .filter(item => item.apartado === 'Consumo de agua m3')
      .map(item => {
        let horaOriginal = '';
        if (item.hora) {
